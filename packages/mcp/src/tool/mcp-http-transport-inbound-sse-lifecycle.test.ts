...
describe('HttpMCPTransport inbound SSE lifecycle', () => {
  ...
  it('does not create a third GET when 202 follows EOF', async () => {
    ...
    try {
      ...
      // A stale reconnect timer must not create another GET after the
      // 202-triggered start has already run.
  ...
  it('uses SSE retry timing and Last-Event-ID when reconnecting', async () => {
    let getCalls = 0;
  ...
  it('does not count EOF cycles against failed-open retries', async () => {
    let getCalls = 0;
  ...
  it('stops after two failed reopen attempts', async () => {
    let getCalls = 0;
    const errors: unknown[] = [];
  ...
  it('cancels a scheduled EOF reconnect on close', async () => {
    let getCalls = 0;
