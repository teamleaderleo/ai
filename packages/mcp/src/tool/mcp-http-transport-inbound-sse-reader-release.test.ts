...
describe('HttpMCPTransport inbound SSE reader ownership', () => {
  beforeEach(() => {
    vi.useFakeTimers();
  });
  ...
  it('releases the reader after EOF before reconnecting', async () => {
    let getCalls = 0;
  ...
  it('releases the reader after close settles a pending read', async () => {
  ...
  it('preserves the read error when cancellation also rejects', async () => {
  ...
  it('does not replace EOF when releaseLock throws', async () => {
    let getCalls = 0;
    const errors: unknown[] = [];
    const reader: ControlledInboundSseReader = {
      read: vi.fn().mockResolvedValue({ done: true, value: undefined }),
