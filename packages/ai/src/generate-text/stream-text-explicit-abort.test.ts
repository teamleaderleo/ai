...
describe('streamText explicit abort terminal settlement', () => {
  it(
    'settles root promises and representative derived getters when abort fires during a pending provider read',
    async () => {
  ...
  it(
    'settles a pre-aborted operation without waiting for provider output',
    async () => {
      const abortController = new AbortController();
  ...
  it(
    'delivers explicit abort to an active local tool and does not report normal completion',
    async () => {
      const abortController = new AbortController();
