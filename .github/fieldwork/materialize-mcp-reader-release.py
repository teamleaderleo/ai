...
open_new = """        } finally {
          try {
            if (this.inboundSseConnection === connection) {
"""
if source.count(open_old) != 1:
    raise SystemExit(
        f'expected one inbound SSE finalizer opening, found {source.count(open_old)}'
    )
source = source.replace(open_old, open_new, 1)

tail_old = """              this.scheduleInboundSseReconnection();
            }
          }
        }
      };
...
if source.count(tail_old) != 1:
    raise SystemExit(
        f'expected one inbound SSE finalizer tail, found {source.count(tail_old)}'
    )
source_path.write_text(source.replace(tail_old, tail_new, 1), encoding='utf-8')

test_path = Path(
    'packages/mcp/src/tool/mcp-http-transport-inbound-sse-reader-release.test.ts'
)
if test_path.exists():
